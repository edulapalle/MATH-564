...
```{r}
#plotting age vs charges with colour coding of smoker 
ggplot(df,aes(charges,age,colour = as.factor(smoker)))+geom_point()
```
```{r}
#plotting age vs charges with colour coding of smoker 
ggplot(df,aes(charges,bmi,colour = as.factor(smoker)))+geom_point()
```

```{r}
#plotting age vs charges with colour coding of smoker 
ggplot(df,aes(charges,children,colour = as.factor(smoker)))+geom_point()
```



```{r}
#pair plot of all variables with colour coding of Smoker.
library(ggplot2)
library(GGally)

ggpairs(df,aes(colour = as.factor(smoker)),upper = list(continuous = wrap("cor", size = 2.5)))
```
